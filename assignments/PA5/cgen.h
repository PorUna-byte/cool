...
private:
   ...
   //Assgin a class_tag for a class_name
...
public:
   ...
   CgenNode* GetClassNode(Symbol class_name){
      GetClassNodes();
      //class_tag is also the index of class_node in m_class_nodes
...
public: 
   ...
   //All methods of a class (Not including inherited ones)
   ...
   //A map from attribute_name to index of the attribute in m_full_attribs
   std::map<Symbol, int> GetAttribIdxTab();
   std::map<Symbol, int> m_attrib_idx_tab;
   //All ancestors,from object to itself
...
private:
   ...
   std::vector<Symbol> m_var_tab;
   std::vector<Symbol> m_param_tab;
...
public: 
   ...
   int LookUpVar(Symbol sym){
      for(int idx=m_var_tab.size()-1;idx>=0;idx--){
         if(m_var_tab[idx]==sym)
            return m_var_tab.size()-1-idx;
      }
      return -1;
   }
   int AddVar(Symbol sym){
      m_var_tab.push_back(sym);
      m_scope_lengths[m_scope_lengths.size()-1]++;
      return m_var_tab.size()-1;
   }
   int LookUpParam(Symbol sym){
      for(int idx=0; idx<m_param_tab.size();idx++){
         if(m_param_tab[idx]==sym)
            return m_param_tab.size()-1-idx;   
      }
      return -1;
   }
   int AddParam(Symbol sym){
      m_param_tab.push_back(sym);
      return m_param_tab.size()-1;
   }
};
