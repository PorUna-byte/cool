...
bool ClassTable::CheckInheritance(Symbol ancestor, Symbol child) {
    ...
    for (; child != No_class && child!=ancestor; child = m_classes[child]->GetParent()) ;
...
void method_class::AddMethodToTable(Symbol class_name) {
    ...
    methodtables[class_name].addid(name, (method_class*)copy_Feature());
}
...
void method_class::CheckFeatureType() {
    ...
    Symbol static_type = return_type;
    // if(return_type==SELF_TYPE)
    //     return_type=curr_class->GetName();
    if (classtable->CheckInheritance(static_type, expr_type) == false) {
...
void attr_class::CheckFeatureType() {
    ...
    Symbol static_type= type_decl==SELF_TYPE? curr_class->GetName():type_decl;
    attribtable.enterscope();
    attribtable.addid(self,new Symbol(curr_class->GetName()));
    Symbol dynamic_type=init->CheckExprType();
    if(dynamic_type!=No_type&&!classtable->CheckInheritance(static_type,dynamic_type))
        classtable->semant_error(curr_class) << "Error! declared is not ancestor of init type. " << std::endl;
...
Symbol dispatch_class::CheckExprType() {
    ...
    Symbol expr_type = expr->CheckExprType();
    Symbol search_type = expr_type;
    // if(classtable->CheckInheritance(curr_class->GetName(),search_type))
    //     search_type=curr_class->GetName();
    // Find the method along the inheritance path.
    // We want the definition in a subclass.
    std::list<Symbol> path = classtable->GetInheritancePath(search_type);
    method_class* method = NULL;
    for (auto iter = path.rbegin(); iter != path.rend(); ++iter) {
        log << "Looking for method in class " << *iter << std::endl;
        if ((method = methodtables[*iter].lookup(name)) != NULL) {
...
Symbol typcase_class::CheckExprType() {
    ...
    for (long unsigned int i = 0; i < branch_types.size() - 1; ++i) {
        for (long unsigned int j = i + 1; j < branch_types.size(); ++j) {
    ...
    type = branch_types[0];
    for (long unsigned int i = 1; i < branch_types.size(); ++i) {
...
Symbol let_class::CheckExprType() {
    ...
    Symbol static_type=type_decl;
    // if(static_type==SELF_TYPE)
    //     static_type=curr_class->GetName();

    Symbol init_type = init->CheckExprType();
    // if there is an initialization expression
    if (init_type != No_type &&!classtable->CheckInheritance(static_type, init_type)){
        classtable->semant_error(curr_class) << "Error! init value is not child." << std::endl;
        error=true;
    }
    attribtable.enterscope();
    attribtable.addid(identifier, new Symbol(static_type));
